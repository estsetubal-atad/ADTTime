...
/*
COMPLEMENTAR. Tentar adicionar à especificação:

int timeCompare(PtTime t1, PtTime t2);
→ Compara dois tempos (semelhante a strcmp): devolve -1, 0 ou 1.

int timeSet(PtTime t, int hour, int minute, int second);
→ Permite alterar os valores de uma estrutura existente, validando-os.

bool timeEquals(PtTime t1, PtTime t2);
→ Verifica se dois tempos são iguais.
